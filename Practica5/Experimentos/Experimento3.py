''' 
Reproduzca el video durante 20s, comenzando en volumen 0 con incrementos
graduales hasta 100% en los primeros 5s; decrementa el volumen gradualemnte
hasta 0 durante los últimos 5s.
Inmediatamente despues reproduce en bucle infinito cada imagen de muestra 
por 3s se detiene al insertar una USB y se reproducen las imágenes de este medio.
...
# === Reproduce el video durante 10 segundos, incrementa y decrementa volumen ===
def play_video(player, video):
  player.set_media(video)
  player.play()

  # Incrementa volumen gradualmente hasta 100%
  for i in range(101):
    player.audio_set_volume(i)
    sleep(0.1)

  # Reproduce al máximo volumen durante 10 segundos
  sleep(10)

  # Decrementa volumen gradualmente hasta 0
